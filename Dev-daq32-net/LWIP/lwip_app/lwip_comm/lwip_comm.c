...
void lwip_comm_default_ip_set(__lwip_dev *lwipx)
{
	...
	//MAC地址(高三位固定为:2.0.0,低三位为STM32唯一ID)
	lwipx->mac[0]=WorkInfo.NetSet.Mac[0];    
	lwipx->mac[1]=WorkInfo.NetSet.Mac[1];    
	lwipx->mac[2]=WorkInfo.NetSet.Mac[2];    
	lwipx->mac[3]=WorkInfo.NetSet.Mac[3];    
	lwipx->mac[4]=WorkInfo.NetSet.Mac[4];    
	lwipx->mac[5]=WorkInfo.NetSet.Mac[5];   
	//本地IP:192.168.1.30
	lwipx->ip[0]=WorkInfo.NetSet.IP[0];   //192;	
	lwipx->ip[1]=WorkInfo.NetSet.IP[1];   //168;
	lwipx->ip[2]=WorkInfo.NetSet.IP[2];   //1;
	lwipx->ip[3]=WorkInfo.NetSet.IP[3];   //30;
	//子网掩码:255.255.255.0
	lwipx->netmask[0]=WorkInfo.NetSet.SubNet[0];   //255;	
	lwipx->netmask[1]=WorkInfo.NetSet.SubNet[1];   //255;
	lwipx->netmask[2]=WorkInfo.NetSet.SubNet[2];   //255;
	lwipx->netmask[3]=WorkInfo.NetSet.SubNet[3];   //0;
	//网关:192.168.1.1
	lwipx->gateway[0]=WorkInfo.NetSet.Gate[0];   //192;	
	lwipx->gateway[1]=WorkInfo.NetSet.Gate[0];   //168;
	lwipx->gateway[2]=WorkInfo.NetSet.Gate[0];   //1;
	lwipx->gateway[3]=WorkInfo.NetSet.Gate[0];   //1;	
